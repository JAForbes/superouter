...
function RouteType({ safeRouteType }){
...
const matchesOr = 
    ({ routeType, tokenized }) => (otherwise, url) => {
...
function type$safe(typeName, cases){
    ...
    if( validated.case === 'N' ){
        ...
        routeType.safe = safeRouteType

        routeType.fold = cases => o => cases[o.case](o.value)

        routeType.matchesOr = matchesOr({tokenized, routeType })        

        return Valid.Y(routeType)
    }
}
